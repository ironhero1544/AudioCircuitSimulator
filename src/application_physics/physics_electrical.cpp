...
void Application::SimulateElectricalImpl() {
  ...
  struct ElectricalCache {
    ...
    std::array<float, kMaxElectricalPorts> net_voltage;
    std::array<bool, kMaxElectricalPorts> net_driven;
    int net_count = 0;
  ...
  auto clear_net_voltages = [&]() {
    for (int i = 0; i < cache.net_count; ++i) {
      cache.net_voltage[i] = 0.0f;
      cache.net_driven[i] = false;
    }
  };

  auto assign_net_driver = [&](int net_id, float drive) {
    if (net_id < 0) {
      return;
    }
    float& slot = cache.net_voltage[net_id];
    if (!cache.net_driven[net_id]) {
      slot = drive;
      cache.net_driven[net_id] = true;
      return;
    }
    if (std::abs(slot - drive) > 0.001f) {
      cache.net_driven[net_id] = false;
      slot = 0.0f;
    }
  };
    ...
    for (int i = 0; i < cache.port_count; ++i) {
      ...
      if (comp_id >= 0 && comp_id <= cache.last_max_component_id) {
        ...
        if (type == ComponentType::POWER_SUPPLY) {
          if (port_id == 0) {
            assign_net_driver(net_id, 24.0f);
          } else if (port_id == 1) {
            assign_net_driver(net_id, 0.0f);
          }
        } else if (type == ComponentType::AUDIO_DC_SOURCE ||
                   type == ComponentType::AUDIO_AC_SOURCE ||
                   type == ComponentType::AUDIO_PULSE_SOURCE) {
          const PlacedComponent* source = FindComponentById(comp_id);
          if (!source) continue;
          if (port_id == 1) {
            assign_net_driver(net_id, 0.0f);
          } else if (port_id == 0) {
            float voltage = 0.0f;
            if (type == ComponentType::AUDIO_DC_SOURCE) {
              voltage = source->internalStates.count(state_keys::kVoltageVolts)
                            ? source->internalStates.at(state_keys::kVoltageVolts)
                            : 5.0f;
            } else {
              const float frequency = std::max(
                  0.01f, source->internalStates.count(state_keys::kFrequencyHz)
                             ? source->internalStates.at(state_keys::kFrequencyHz)
                             : 1000.0f);
              const double cycle = std::fmod(
                  source_time_seconds * static_cast<double>(frequency), 1.0);
              if (type == ComponentType::AUDIO_AC_SOURCE) {
                const float rms = source->internalStates.count(
                                      state_keys::kVoltageVolts)
                                      ? source->internalStates.at(
                                            state_keys::kVoltageVolts)
                                      : 1.0f;
                const float phase = source->internalStates.count(
                                        state_keys::kPhaseDegrees)
                                        ? source->internalStates.at(
                                              state_keys::kPhaseDegrees)
                                        : 0.0f;
                voltage = rms * 1.41421356f * std::sin(
                    6.283185307179586 * cycle +
                    static_cast<double>(phase) * 0.017453292519943);
              } else {
                const float duty = std::clamp(
                    source->internalStates.count(state_keys::kDutyCyclePercent)
                        ? source->internalStates.at(
                              state_keys::kDutyCyclePercent)
                        : 50.0f,
                    0.01f, 99.99f);
                const bool high = cycle < static_cast<double>(duty / 100.0f);
                const char* key = high ? state_keys::kPulseHighVolts
                                       : state_keys::kPulseLowVolts;
                voltage = source->internalStates.count(key)
                              ? source->internalStates.at(key)
                              : (high ? 5.0f : 0.0f);
              }
            }
            assign_net_driver(net_id, voltage);
          }
          ...
          if (y_state) {
            assign_net_driver(net_id, GetPlcOutputOnVoltage());
          }
        }
      }
    }
    ...
    for (const auto& comp : placed_components_) {
      ...
      for (const auto& binding : comp.rtlPinBindings) {
        ...
        if (value == RtlLogicValue::ONE) {
          assign_net_driver(net_id, GetRtlOutputHighVoltage(comp));
  ...
  auto update_port_voltages = [&]() {
    for (int i = 0; i < cache.port_count; ++i) {
      int net_id = cache.port_net_id[i];
      float voltage = -1.0f;
      if (net_id >= 0 && cache.net_driven[net_id]) {
        voltage = cache.net_voltage[net_id];
      }
