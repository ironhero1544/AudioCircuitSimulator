...
std::string ProjectFileManager::GenerateManifest(const ProjectInfo& info) {
  ...
  manifest["audio_engine_version"] =
      std::to_string(info.audioEngineVersion);
...
bool ProjectFileManager::ParseManifest(const std::string& manifestContent,
  ...
  try {
    ...
    const std::string audio_engine_version =
        manifest["audio_engine_version"];
    info.audioEngineVersion = audio_engine_version.empty()
                                  ? 2
                                  : std::max(2, std::stoi(audio_engine_version));
