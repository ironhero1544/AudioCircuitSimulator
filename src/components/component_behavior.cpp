...
ComponentInteractionResult HandlePotentiometerMouseWheel(
    const PlacedComponent& comp, ImVec2 world_pos, const ImGuiIO& io) {
  (void)world_pos;
  if (io.MouseWheel == 0.0f) return UnhandledInteraction();
  float current = comp.internalStates.count(state_keys::kWiperPosition)
                      ? comp.internalStates.at(state_keys::kWiperPosition)
                      : 0.5f;
  float step = io.KeyCtrl ? 0.005f : 0.025f;
  current += io.MouseWheel > 0.0f ? step : -step;
  ComponentInteractionCommand command;
  command.type = ComponentInteractionCommandType::SetPotentiometerWiper;
  command.scalar = std::clamp(current, 0.0f, 1.0f);
  return MakeHandledInteraction(command);
}

bool BuildPotentiometerTooltip(const PlacedComponent& comp, ImVec2 world_pos) {
  (void)world_pos;
  const float value = comp.internalStates.count(state_keys::kWiperPosition)
                          ? comp.internalStates.at(state_keys::kWiperPosition)
                          : 0.5f;
  ImGui::BeginTooltip();
  ImGui::Text("Wiper: %.1f%%", value * 100.0f);
  ImGui::TextDisabled("Use mouse wheel to adjust.");
  ImGui::EndTooltip();
  return true;
}
  ...
  for (const auto& command : result.commands) {
    switch (command.type) {
      ...
      case ComponentInteractionCommandType::SetFlowSetting:
        ...
        break;
      case ComponentInteractionCommandType::SetPotentiometerWiper:
        comp->internalStates[state_keys::kWiperPosition] = command.scalar;
        break;
...
const ComponentBehavior* GetComponentBehavior(ComponentType type) {
  constexpr int kComponentTypeCount = 37;
  static_assert(static_cast<int>(ComponentType::AUDIO_BJT_PNP) ==
  ...
  static const ComponentBehavior kBehaviors[kComponentTypeCount] = {
      ...
      {nullptr, nullptr, nullptr, nullptr, nullptr},
      // AUDIO_POTENTIOMETER
      {nullptr, nullptr, nullptr, HandlePotentiometerMouseWheel,
       BuildPotentiometerTooltip},
      // AUDIO_CAPACITOR
      {nullptr, nullptr, nullptr, nullptr, nullptr},
      // AUDIO_INDUCTOR
      {nullptr, nullptr, nullptr, nullptr, nullptr},
