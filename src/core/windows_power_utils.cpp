...
static bool g_simulation_running_power = false;

DWORD QueryWindowsEfficiencyModeStateMask() {
  if (g_simulation_running_power) {
    return 0;
  }
...
void SetExecutionStateKeepAlive(bool enable) {
#ifdef _WIN32
  if (enable) {
...
void SetSimulationRunningPowerState(bool running) {
#ifdef _WIN32
  g_simulation_running_power = running;
#endif
  SetExecutionStateKeepAlive(running);
}
