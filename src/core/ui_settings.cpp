...
bool BuildUserSettingsPath(char* out_path, size_t out_size) {
  ...
  if (GetLocalAppDataUtf8(base_dir, sizeof(base_dir))) {
    std::snprintf(out_path, out_size,
                  "%s\\AudioCircuitSimulator\\ui_settings.cfg",
                  base_dir);
    return true;
  }
#endif
...
void SetDefaultUiSettings(UiSettings* settings) {
  ...
  settings->audio_min_volume_percent = 0.0f;
  settings->audio_max_volume_percent = 100.0f;
  settings->audio_buffer_ms = 80;
  settings->audio_input_device_id.clear();
  settings->audio_output_device_id.clear();
...
bool LoadUiSettings(UiSettings* settings) {
  ...
  while (std::fgets(line, sizeof(line), file)) {
    ...
    } else if (std::strncmp(line, "audio_min_volume_percent=", 25) == 0) {
      std::sscanf(line + 25, "%f", &settings->audio_min_volume_percent);
    } else if (std::strncmp(line, "audio_max_volume_percent=", 25) == 0) {
      std::sscanf(line + 25, "%f", &settings->audio_max_volume_percent);
    } else if (std::strncmp(line, "audio_buffer_ms=", 16) == 0) {
      std::sscanf(line + 16, "%d", &settings->audio_buffer_ms);
    } else if (std::strncmp(line, "audio_input_device_id=", 22) == 0) {
      settings->audio_input_device_id = line + 22;
    } else if (std::strncmp(line, "audio_output_device_id=", 23) == 0) {
      settings->audio_output_device_id = line + 23;
    }
  }
  std::fclose(file);
  ...
  settings->audio_min_volume_percent =
      std::clamp(settings->audio_min_volume_percent, 0.0f, 100.0f);
  settings->audio_max_volume_percent = std::clamp(
      settings->audio_max_volume_percent,
      settings->audio_min_volume_percent, 100.0f);
  settings->audio_buffer_ms =
      std::clamp(settings->audio_buffer_ms, 10, 500);
  return true;
}
...
bool SaveUiSettings(const UiSettings& settings) {
  ...
  std::fprintf(file, "audio_min_volume_percent=%.2f\n",
               settings.audio_min_volume_percent);
  std::fprintf(file, "audio_max_volume_percent=%.2f\n",
               settings.audio_max_volume_percent);
  std::fprintf(file, "audio_buffer_ms=%d\n", settings.audio_buffer_ms);
  std::fprintf(file, "audio_input_device_id=%s\n",
               settings.audio_input_device_id.c_str());
  std::fprintf(file, "audio_output_device_id=%s\n",
               settings.audio_output_device_id.c_str());
  std::fclose(file);
  return true;
}
