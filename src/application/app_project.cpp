...
bool ComponentTypeFromString(const std::string& value, ComponentType* out) {
  ...
  } else if (value == "AUDIO_SOURCE") {
    *out = ComponentType::AUDIO_SOURCE;
  } else if (value == "AUDIO_ADC") {
    *out = ComponentType::AUDIO_ADC;
  } else if (value == "AUDIO_RESISTOR") {
    *out = ComponentType::AUDIO_RESISTOR;
  } else if (value == "AUDIO_POTENTIOMETER") {
    *out = ComponentType::AUDIO_POTENTIOMETER;
  } else if (value == "AUDIO_CAPACITOR") {
    *out = ComponentType::AUDIO_CAPACITOR;
  } else if (value == "AUDIO_INDUCTOR") {
    *out = ComponentType::AUDIO_INDUCTOR;
  } else if (value == "AUDIO_OP_AMP") {
    *out = ComponentType::AUDIO_OP_AMP;
  } else if (value == "AUDIO_DAC") {
    *out = ComponentType::AUDIO_DAC;
  } else if (value == "AUDIO_SPEAKER") {
    *out = ComponentType::AUDIO_SPEAKER;
  } else if (value == "AUDIO_GROUND") {
    *out = ComponentType::AUDIO_GROUND;
  } else if (value == "AUDIO_DC_SOURCE") {
    *out = ComponentType::AUDIO_DC_SOURCE;
  } else if (value == "AUDIO_AC_SOURCE") {
    *out = ComponentType::AUDIO_AC_SOURCE;
  } else if (value == "AUDIO_PULSE_SOURCE") {
    *out = ComponentType::AUDIO_PULSE_SOURCE;
  } else if (value == "AUDIO_DIODE") {
    *out = ComponentType::AUDIO_DIODE;
  } else if (value == "AUDIO_BJT_NPN") {
    *out = ComponentType::AUDIO_BJT_NPN;
  } else if (value == "AUDIO_BJT_PNP") {
    *out = ComponentType::AUDIO_BJT_PNP;
  } else {
    return false;
  }
  return true;
...
bool IsPersistentAudioParameter(const std::string& key) {
  return key == state_keys::kAudioLevel ||
         key == state_keys::kBitDepth ||
...
std::string Application::SerializeLayoutJson() const {
  ...
  for (const auto& comp : placed_components_) {
    ...
    json audio_parameters = json::object();
    for (const auto& [key, value] : comp.internalStates) {
      if (IsPersistentAudioParameter(key)) {
        audio_parameters[key] = value;
      }
    }
    if (!audio_parameters.empty()) {
      item["audio_parameters"] = std::move(audio_parameters);
    }
...
bool Application::DeserializeLayoutJson(const std::string& layout_json) {
  ...
  if (components_it != root.end() && components_it->is_array()) {
    for (const auto& item : *components_it) {
      ...
      if (item.contains("audio_parameters") &&
          item["audio_parameters"].is_object()) {
        for (auto parameter = item["audio_parameters"].begin();
             parameter != item["audio_parameters"].end(); ++parameter) {
          if (IsPersistentAudioParameter(parameter.key()) &&
              parameter.value().is_number()) {
            comp.internalStates[parameter.key()] =
...
bool Application::LoadProject(const std::string& filePath) {
  ...
  try {
    ...
    if (rtl_project_manager_) {
      rtl_project_manager_->ResetProjectAudioComponentShells();
    }
