...
bool ContainsAsciiIgnoreCase(std::string text, const std::string& needle) {
  ...
  std::transform(lowered_needle.begin(), lowered_needle.end(),
                 lowered_needle.begin(), [](unsigned char ch) {
                   return static_cast<char>(std::tolower(ch));
                 });
  return text.find(lowered_needle) != std::string::npos;
}
...
void Application::RequestRtlAnalyzeAsync(const std::string& moduleId) {
  ...
  for (auto& component : placed_components_) {
    if (component.rtlModuleId == moduleId) {
      RefreshRtlComponentFromLibrary(&component);
    }
  }
  ...
  rtl_async_task_future_ =
      std::async(std::launch::async, [this, entryCopy]() mutable {
        ...
        result.success = rtl_project_manager_->AnalyzeDetached(&result.entry);
...
void Application::RequestRtlBuildAsync(const std::string& moduleId) {
  ...
  rtl_async_task_future_ =
      std::async(std::launch::async, [this, entryCopy]() mutable {
        ...
        result.success = rtl_project_manager_->BuildDetached(&result.entry);
...
void Application::RequestRtlTestbenchAsync(const std::string& moduleId) {
  ...
  rtl_async_task_future_ =
      std::async(std::launch::async, [this, entryCopy]() mutable {
        ...
        result.success =
            rtl_project_manager_->RunTestbenchDetached(&result.entry);
...
void Application::PollRtlAsyncTask() {
  ...
  if (result.taskKind == "analyze") {
    ...
    // RunVerilatorBuild upgrades the generated audio worker protocol on the
    // detached copy.  Keep that negotiated protocol with the installed
    // artifact; otherwise a successful rebuild still launches the new worker
    // as the legacy text protocol and the audio engine rejects it.
    target->workerProtocolVersion = result.entry.workerProtocolVersion;
    ClearRtlTestbenchResult(target);
  ...
  if (result.taskKind == "analyze" || result.taskKind == "build") {
...
void Application::InvalidateRtlModuleRuntimeState(const std::string& moduleId) {
  ...
  // This function is called both from the UI/build path and recursively from
  // the simulation path.  Keep component state invalidation atomic with the
  // rest of the simulation's placed-component access.
...
void Application::UpdateRtlSimulation(
  ...
  for (auto& comp : placed_components_) {
    ...
    // Audio DACs are clocked at audio rate by the WASAPI block path. Sending
    // frame-rate EVAL commands to the same worker would corrupt its I2S state.
    if (comp.rtlModuleId == "audio_shell_dac") {
      continue;
    }
...
void Application::RefreshRtlComponentFromLibrary(PlacedComponent* comp) {
  ...
  // Zero-pin shells still need the original RTL IC body size. Hit testing,
  // selection, dragging and context actions all use PlacedComponent::size.
  const ComponentDefinition* shell_appearance =
      GetAudioRtlShellAppearance(entry->moduleId);
  const ComponentDefinition* definition =
      shell_appearance ? shell_appearance
                       : GetComponentDefinition(ComponentType::RTL_MODULE);
  if (shell_appearance) {
    const bool needs_initial_state =
        comp->size.width <= 0.0f || comp->size.height <= 0.0f;
    comp->size = definition->size;
    if (needs_initial_state && definition->InitDefaultState) {
      definition->InitDefaultState(comp);
    }
  } else if (definition &&
             (comp->size.width <= 0.0f || comp->size.height <= 0.0f)) {
    comp->size = definition->size;
    if (definition->InitDefaultState) {
      definition->InitDefaultState(comp);
    }
  }
  const bool artifact_ready =
      entry->analyzeSuccess && entry->buildSuccess &&
  ...
  const size_t input_count = std::count_if(
      comp->runtimePorts.begin(), comp->runtimePorts.end(),
      [](const Port& port) { return port.isInput; });
  const size_t output_count = comp->runtimePorts.size() - input_count;
  const size_t max_side_count = std::max(input_count, output_count);
  if (max_side_count > 1) {
    const float minimum_height =
        (shell_appearance ? 56.0f : 104.0f) +
        18.0f * static_cast<float>(max_side_count - 1);
    comp->size.height = std::max(comp->size.height, minimum_height);
    if (!shell_appearance) {
      comp->size.width = std::max(comp->size.width, 260.0f);
    } else if (max_side_count > 8) {
      comp->size.width = std::max(comp->size.width, 190.0f);
    }
  }
  if (shell_appearance) {
    size_t input_index = 0;
    size_t output_index = 0;
    const float top_margin = std::min(28.0f, comp->size.height * 0.28f);
    const float usable_height =
        std::max(1.0f, comp->size.height - top_margin * 2.0f);
    for (Port& port : comp->runtimePorts) {
      const size_t count = port.isInput ? input_count : output_count;
      size_t& index = port.isInput ? input_index : output_index;
      const float fraction =
          count > 1 ? static_cast<float>(index) /
                          static_cast<float>(count - 1)
                    : 0.5f;
      port.relativePos.x = port.isInput ? 0.0f : comp->size.width;
      port.relativePos.y = top_margin + usable_height * fraction;
      ++index;
    }
  }
...
void Application::PlaceRtlLibraryComponent(const std::string& moduleId,
  ...
  comp.customLabel = entry->displayName;
  RefreshRtlComponentFromLibrary(&comp);
  if (comp.rtlModuleId == "audio_shell_amplifier") {
    comp.internalStates[state_keys::kGain] = 4.0f;
    comp.internalStates[state_keys::kGainBandwidthHz] = 100000.0f;
    comp.internalStates[state_keys::kOutputResistanceOhms] = 0.1f;
    comp.internalStates[state_keys::kOutputHeadroomVolts] = 1.0f;
  }
  comp.internalStates[kRtlClockAccumKey] = 0.0f;
  comp.internalStates[kRtlClockLevelKey] = 0.0f;
  comp.internalStates[kRtlResetElapsedKey] = 0.0f;
...
void Application::OpenRtlComponentEditor(int component_id) {
  if (!rtl_project_manager_) {
    return;
  }
  ...
  if (component == placed_components_.end() ||
      component->type != ComponentType::RTL_MODULE ||
      component->rtlModuleId.empty()) {
    return;
  }
  RtlLibraryEntry* entry =
      rtl_project_manager_->FindEntryById(component->rtlModuleId);
  if (!entry) {
    return;
  }
  selected_rtl_module_id_ = entry->moduleId;
  rtl_editor_focus_module_id_ = entry->moduleId;
  if (!entry->sourceFiles.empty()) {
    selected_rtl_source_path_ = entry->sourceFiles.front().path;
    rtl_editor_buffer_ = entry->sourceFiles.front().content;
  } else {
  ...
  rtl_status_message_ =
      "Edit Verilog, then run Analyze and Build to generate component pins.";
...
bool Application::InstallRtlToolchain() {
  if (rtl_tool_setup_running_) {
    return false;
  }
  rtl_tool_setup_running_ = true;
  rtl_tool_setup_phase_.store(1, std::memory_order_relaxed);
  rtl_tool_setup_operation_ = RtlToolSetupOperation::kInstallAll;
  rtl_status_message_ = TR(
      "ui.rtl.status_installing_all",
      "Installing WSL RTL tools and Virtual Audio Cable 4.71 Lite...");
  rtl_tool_setup_future_ = std::async(std::launch::async, [this]() {
    // Start the Windows-side driver first. This produces an immediate UAC
    // prompt instead of making the user wait behind a possibly slow apt run.
    rtl_tool_setup_phase_.store(2, std::memory_order_relaxed);
    AudioRtlProcessResult audio = InstallVirtualAudioDriver();

    rtl_tool_setup_phase_.store(3, std::memory_order_relaxed);
    AudioRtlProcessResult wsl = AudioRtlWslToolchain::Install();

    rtl_tool_setup_phase_.store(4, std::memory_order_relaxed);
    AudioRtlProcessResult combined;
    combined.started = wsl.started || audio.started;
    combined.timed_out = wsl.timed_out || audio.timed_out;
    combined.exit_code = (wsl.Succeeded() && audio.Succeeded()) ? 0U : 1U;
    combined.output = "Virtual Audio Cable:\n" +
        (!audio.output.empty() ? audio.output : audio.error) +
        "\n\nWSL tools:\n" +
        (!wsl.output.empty() ? wsl.output : wsl.error);
    if (!combined.Succeeded()) combined.error = combined.output;
    return combined;
  });
  return true;
}

bool Application::RemoveRtlToolchain() {
  if (rtl_tool_setup_running_) {
    return false;
  }

  // Clear the VAC sink selection on the UI thread so the regular update loop
  // cannot reconnect while the uninstall task is running. The potentially
  // blocking bridge join is performed by the worker instead of freezing ImGui.
  ui_settings_.audio_input_device_id.clear();
  SaveUiSettings(ui_settings_);

  rtl_tool_setup_running_ = true;
  rtl_tool_setup_phase_.store(1, std::memory_order_relaxed);
  rtl_tool_setup_operation_ = RtlToolSetupOperation::kRemoveAll;
  rtl_status_message_ = TR(
      "ui.rtl.status_removing_all",
      "Removing Verilator, g++, make, and Virtual Audio Cable...");
  rtl_tool_setup_future_ = std::async(std::launch::async, [this]() {
    ShutdownAudioStreamBridge();

    // Remove VAC first so the administrator prompt appears immediately and
    // is not hidden behind a slow/locked apt operation in WSL.
    rtl_tool_setup_phase_.store(2, std::memory_order_relaxed);
    AudioRtlProcessResult audio = RemoveVirtualAudioDriver();

    rtl_tool_setup_phase_.store(3, std::memory_order_relaxed);
    AudioRtlProcessResult wsl = AudioRtlWslToolchain::Remove();
...
void Application::PollRtlToolSetup() {
  if (!rtl_tool_setup_running_ || !rtl_tool_setup_future_.valid() ||
      rtl_tool_setup_future_.wait_for(std::chrono::milliseconds(0)) !=
          std::future_status::ready) {
    return;
  }

  AudioRtlProcessResult result = rtl_tool_setup_future_.get();
  const RtlToolSetupOperation operation = rtl_tool_setup_operation_;
  rtl_tool_setup_running_ = false;
  rtl_tool_setup_phase_.store(0, std::memory_order_relaxed);
  rtl_tool_setup_operation_ = RtlToolSetupOperation::kNone;

  if (operation == RtlToolSetupOperation::kInstallAll && result.Succeeded()) {
    const auto& audio_devices = GetAudioOutputDevices(true);
    bool selected_sink_exists = false;
    for (const AudioOutputDevice& device : audio_devices) {
      if (!device.id.empty() &&
          device.id == ui_settings_.audio_input_device_id) {
        selected_sink_exists = true;
        break;
      }
    }
    if (!selected_sink_exists) {
      for (const AudioOutputDevice& device : audio_devices) {
        if (!device.id.empty() &&
            ContainsAsciiIgnoreCase(device.name, "virtual audio cable")) {
  ...
  if (result.Succeeded()) {
    switch (operation) {
      case RtlToolSetupOperation::kInstallAll:
        rtl_status_message_ = TR(
            "ui.rtl.status_install_all_done",
            "Tools installed. If the virtual audio device is not visible yet, restart Windows once.");
        break;
      case RtlToolSetupOperation::kRemoveAll:
        rtl_status_message_ = TR(
            "ui.rtl.status_remove_all_done",
            "RTL tools and Virtual Audio Cable were removed. WSL/Ubuntu was kept.");
        if (!result.output.empty()) {
          rtl_status_message_ += "\n";
          rtl_status_message_ += result.output;
        }
        break;
      case RtlToolSetupOperation::kNone:
        rtl_status_message_ = result.output;
        break;
    }
  } else {
    rtl_status_message_ = !result.output.empty() ? result.output : result.error;
  }
...
void Application::RenderRtlToolchainPanel() {
  ...
  if (!rtl_toolchain_status_valid_) {
    ImGui::TextWrapped(
        "%s", "Loading the WSL2 Verilator component backend.");
  ...
  ImGui::Separator();
  ImGui::TextWrapped(
      "%s", TR("ui.rtl.backend_scope",
                "Only Verilog electronic components use this WSL backend. The existing wiring and other component systems are unchanged."));
  const bool virtual_audio_ready = IsVirtualAudioDriverInstalled();
  plc::FormatString(buf, sizeof(buf), "ui.rtl.vac_status",
                    "Virtual Audio Cable: %s",
                    virtual_audio_ready ? TR("ui.rtl.found", "Found")
                                        : TR("ui.rtl.missing", "Missing"));
  ImGui::TextUnformatted(buf);
  ImGui::TextWrapped(
      "%s", TR("ui.rtl.toolchain_policy",
                "Verilog components use Verilator, g++, and make in WSL. Install Tools also prepares the official VAC 4.71 Lite installer."));
  ImGui::TextWrapped(
      "%s", TR("ui.rtl.delete_policy",
                "Delete Tools removes Verilator, g++, make, and VAC. WSL/Ubuntu itself is kept."));

  if (rtl_tool_setup_running_) {
    const int setup_phase = rtl_tool_setup_phase_.load(std::memory_order_relaxed);
    const char* phase_text = TR("ui.rtl.tool_setup_running",
                                "Tool setup is running...");
    if (setup_phase == 2) {
      phase_text = TR("ui.rtl.tool_setup_vac",
                      "Working on Virtual Audio Cable. Check the Windows administrator prompt if it is visible.");
    } else if (setup_phase == 3) {
      phase_text = TR("ui.rtl.tool_setup_wsl",
                      "Working on Verilator/g++/make in WSL...");
    } else if (setup_phase == 4) {
      phase_text = TR("ui.rtl.tool_setup_finishing",
                      "Finishing tool setup and verifying the result...");
    }
    ImGui::TextWrapped("%s", phase_text);
    ImGui::BeginDisabled();
  }

  if (ImGui::Button(TR("ui.rtl.install_toolchain", "Install Tools"))) {
    InstallRtlToolchain();
  }
  ImGui::SameLine();
  ...
  ImGui::SameLine();
  const std::string delete_popup_title =
      std::string(TR("ui.rtl.delete_confirm_title", "Delete RTL Tools")) +
      "##RtlDeleteToolsConfirm";
  if (ImGui::Button(TR("ui.rtl.delete_toolchain", "Delete Tools"))) {
    ImGui::OpenPopup(delete_popup_title.c_str());
  }
  ...
  if (ImGui::BeginPopupModal(delete_popup_title.c_str(), nullptr,
                             ImGuiWindowFlags_AlwaysAutoResize)) {
    ImGui::TextWrapped(
        "%s", TR("ui.rtl.delete_confirm_body",
                  "Close applications that are using VAC, then remove Verilator, g++, and make from the default WSL distribution and uninstall Virtual Audio Cable? WSL/Ubuntu itself will remain installed."));
    ImGui::Spacing();
    if (ImGui::Button(TR("ui.rtl.delete_confirm_yes", "Delete All"),
                      ImVec2(120.0f, 0.0f))) {
...
void Application::RenderRtlEditor() {
  ...
  ImGui::SameLine();
  // Keep the disabled scope decision stable for this frame. Clicking one of
  // these buttons starts an async task immediately and changes
  // rtl_async_task_running_; re-evaluating it at EndDisabled() would unbalance
  // ImGui's disabled stack.
  const bool disable_rtl_task_buttons =
      rtl_async_task_running_ || !hasSourceFiles;
  if (disable_rtl_task_buttons) {
    ImGui::BeginDisabled();
  }
  ...
  if (editorChanged && current_source) {
    ...
    InvalidateRtlModuleRuntimeState(entry->moduleId);
    for (auto& component : placed_components_) {
      if (component.rtlModuleId == entry->moduleId) {
        RefreshRtlComponentFromLibrary(&component);
      }
    }
  }
...
void Application::RenderRtlLibraryPanel() {
  ...
  const bool focused_component_editor =
      !rtl_editor_focus_module_id_.empty();
  const RtlLibraryEntry* focused_entry =
      focused_component_editor && rtl_project_manager_
          ? rtl_project_manager_->FindEntryById(rtl_editor_focus_module_id_)
          : nullptr;
  if (focused_component_editor && !focused_entry) {
    rtl_editor_focus_module_id_.clear();
  }
  const bool isolate_component = focused_entry != nullptr;
  if (!isolate_component && !selected_rtl_module_id_.empty()) {
    const RtlLibraryEntry* selected_entry =
        rtl_project_manager_->FindEntryById(selected_rtl_module_id_);
    if (selected_entry &&
        GetAudioRtlShellAppearance(selected_entry->moduleId)) {
      selected_rtl_module_id_.clear();
      selected_rtl_source_path_.clear();
      rtl_editor_buffer_.clear();
      for (const RtlLibraryEntry& candidate :
           rtl_project_manager_->GetLibrary()) {
        if (GetAudioRtlShellAppearance(candidate.moduleId)) {
          continue;
        }
        selected_rtl_module_id_ = candidate.moduleId;
        if (!candidate.sourceFiles.empty()) {
          SelectRtlSource(candidate.sourceFiles.front(),
                          &selected_rtl_source_path_, &rtl_editor_buffer_);
        }
        break;
      }
  ...
  std::string window_title =
      isolate_component
          ? focused_entry->displayName + " - Verilog Editor###RtlLibrary"
          : std::string(TR("ui.rtl.library_title", "RTL Library")) +
                "###RtlLibrary";
  ...
  if (isolate_component) {
    ImGui::TextDisabled("Component: %s", focused_entry->displayName.c_str());
    ImGui::SameLine();
    if (ImGui::Button(TR("ui.rtl.toolchain", "Toolchain"))) {
      show_rtl_toolchain_panel_ = true;
    }
  } else if (ImGui::Button(TR("ui.rtl.new_module", "New RTL Module"))) {
    RtlLibraryEntry* entry =
  ...
  if (!isolate_component) ImGui::SameLine();
  if (!isolate_component && !selected_rtl_module_id_.empty() &&
  ...
  if (!isolate_component) ImGui::SameLine();
  if (!isolate_component &&
      ImGui::Button(TR("ui.rtl.import_component", "Import Component"))) {
  ...
  if (!isolate_component) ImGui::SameLine();
  if (!isolate_component &&
      ImGui::Button(TR("ui.rtl.toolchain", "Toolchain"))) {
    show_rtl_toolchain_panel_ = true;
  }
  ...
  for (auto& entry : rtl_project_manager_->GetLibrary()) {
    if (isolate_component && entry.moduleId != rtl_editor_focus_module_id_) {
      continue;
    }
    if (!isolate_component && GetAudioRtlShellAppearance(entry.moduleId)) {
      continue;
    }
