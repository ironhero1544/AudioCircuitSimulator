...
bool HasAudioComponent(const std::vector<PlacedComponent>& components,
                       ComponentType type) {
  return std::any_of(components.begin(), components.end(),
                     [type](const PlacedComponent& component) {
                       return component.type == type;
                     });
}

bool HasAudioDac(const std::vector<PlacedComponent>& components) {
  return std::any_of(
      components.begin(), components.end(), [](const PlacedComponent& component) {
        return component.type == ComponentType::AUDIO_DAC ||
               (component.type == ComponentType::RTL_MODULE &&
                component.rtlModuleId == "audio_shell_dac");
      });
}

int CountDacBits(std::uint16_t mask) {
  int count = 0;
  while (mask != 0) {
    count += mask & 1U;
...
void HelpBullet(const char* text) {
  ImGui::Bullet();
  ImGui::SameLine();
  ImGui::TextWrapped("%s", text);
}

void RenderCanvasCircuitHelpTooltip(
    const std::vector<PlacedComponent>& components,
    const AudioCircuitStatus& status, float layout_scale) {
  const bool has_source =
      HasAudioComponent(components, ComponentType::AUDIO_SOURCE);
  const bool has_speaker =
      HasAudioComponent(components, ComponentType::AUDIO_SPEAKER);
  const bool has_ground =
      HasAudioComponent(components, ComponentType::AUDIO_GROUND);
  const bool has_dac = HasAudioDac(components);
  const bool has_npn =
      HasAudioComponent(components, ComponentType::AUDIO_BJT_NPN);
  const bool has_pnp =
      HasAudioComponent(components, ComponentType::AUDIO_BJT_PNP);
  const bool speaker_risk = status.left.speaker_damage_risk ||
                            status.right.speaker_damage_risk;

  ImGui::SetNextWindowSizeConstraints(ImVec2(360.0f * layout_scale, 0.0f),
                                      ImVec2(520.0f * layout_scale,
                                             760.0f * layout_scale));
  ImGui::BeginTooltip();
  ImGui::PushTextWrapPos(ImGui::GetCursorPosX() + 470.0f * layout_scale);
  ImGui::TextUnformatted(
      TR("ui.wiring.help.title", "Circuit tutorial and diagnostics"));
  ImGui::Separator();

  ImGui::TextUnformatted(
      TR("ui.wiring.help.current_problem", "Current circuit status"));
  if (components.empty()) {
    ImGui::TextColored(
        ImVec4(0.95f, 0.62f, 0.12f, 1.0f), "%s",
        TR("ui.wiring.help.empty",
           "The canvas is empty. Place the digital audio source first."));
  } else if (!has_source || !has_speaker || !has_ground) {
    ImGui::TextColored(
        ImVec4(0.90f, 0.25f, 0.18f, 1.0f), "%s",
        TR("ui.wiring.help.required_missing",
           "The required source, speaker, or analog return is missing."));
    if (!has_source) {
      HelpBullet(TR("ui.wiring.help.need_source",
                    "Add PC Digital Audio as the signal source."));
    }
    if (!has_speaker) {
      HelpBullet(TR("ui.wiring.help.need_speaker",
                    "Add a stereo speaker as the load."));
    }
    if (!has_ground) {
      HelpBullet(TR("ui.wiring.help.need_ground",
                    "Add Analog Ground and wire both speaker returns."));
    }
  } else if (!has_dac) {
    ImGui::TextColored(
        ImVec4(0.90f, 0.25f, 0.18f, 1.0f), "%s",
        TR("ui.wiring.help.digital_needs_dac",
           "PC audio is digital; an analog circuit cannot use it directly."));
    HelpBullet(TR("ui.wiring.help.need_dac",
                  "Add a DAC, edit its Verilog, then run Analyze/Build."));
  } else if (!status.route_complete) {
    const char* route_problem =
        TR("ui.wiring.help.route_open",
           "The powered audio path is incomplete.");
    if (status.diagnosis.find("analyzed and built") != std::string::npos) {
      route_problem = TR("ui.wiring.help.problem_rtl_not_built",
                         "The DAC RTL has not been analyzed and built.");
    } else if (status.diagnosis.find("Wire all 16") != std::string::npos) {
      route_problem = TR("ui.wiring.help.problem_dac_bus",
                         "The discrete DAC output bus is not wired.");
    } else if (status.diagnosis.find("DC supply short") !=
               std::string::npos) {
      route_problem = TR("ui.wiring.help.problem_supply_short",
                         "The DC supply +V and return nets are shorted.");
    } else if (status.diagnosis.find("line-level only") !=
               std::string::npos) {
      route_problem = TR("ui.wiring.help.problem_line_level",
                         "The current amplifier is line-level and cannot drive a speaker.");
    } else if (status.diagnosis.find("complementary class-AB") !=
               std::string::npos) {
      route_problem = TR("ui.wiring.help.problem_unpowered_stage",
                         "The complementary Class-AB output stage is not correctly powered or driven.");
    }
    ImGui::TextColored(ImVec4(0.90f, 0.25f, 0.18f, 1.0f), "%s",
                       route_problem);
    if (status.diagnosis.find("analyzed and built") != std::string::npos) {
      HelpBullet(TR("ui.wiring.help.need_rtl_build",
                    "Open the DAC editor and run Analyze/Build with Verilator."));
    } else if (status.diagnosis.find("DC supply short") !=
               std::string::npos) {
      HelpBullet(TR("ui.wiring.help.need_remove_short",
                    "Separate the supply +V and return nets before starting simulation."));
    } else if (!has_npn || !has_pnp) {
      HelpBullet(TR("ui.wiring.help.need_output_pair",
                    "Add complementary NPN and PNP output transistors after the powered driver."));
    } else {
      char bus_hint[256] = {};
      FormatString(bus_hint, sizeof(bus_hint), "ui.wiring.help.need_bus_fmt",
                   "Check the DAC buses (L %d/16, R %d/16), powered driver, output pair, and speaker connection.",
                   CountDacBits(status.left.dac_connected_mask),
                   CountDacBits(status.right.dac_connected_mask));
      HelpBullet(bus_hint);
    }
  } else if (!status.return_complete) {
    ImGui::TextColored(
        ImVec4(0.90f, 0.25f, 0.18f, 1.0f), "%s",
        TR("ui.wiring.help.return_open",
           "A speaker negative/return path is open, so that channel is muted."));
    HelpBullet(TR("ui.wiring.help.need_return",
                  "Wire LEFT_NEG and RIGHT_NEG to Analog Ground."));
  } else if (speaker_risk) {
    ImGui::TextColored(
        ImVec4(0.95f, 0.45f, 0.10f, 1.0f), "%s",
        TR("ui.wiring.help.speaker_risk",
           "The route works, but the output stage can damage the speaker."));
    if (!status.left.dc_blocking_capacitor_present ||
        !status.right.dc_blocking_capacitor_present) {
      HelpBullet(TR("ui.wiring.help.need_dc_block",
                    "Add a 1000 uF or larger series DC-blocking capacitor per channel."));
    }
    if (!status.left.emitter_ballast_present ||
        !status.right.emitter_ballast_present) {
      HelpBullet(TR("ui.wiring.help.need_ballast",
                    "Add 0.1-0.47 ohm emitter ballast resistors to the NPN/PNP output pair."));
    }
  } else if (!status.left.route_complete || !status.right.route_complete) {
    ImGui::TextColored(
        ImVec4(0.95f, 0.62f, 0.12f, 1.0f), "%s",
        TR("ui.wiring.help.one_channel",
           "Only one channel reaches the speaker."));
    HelpBullet(TR("ui.wiring.help.need_stereo_symmetry",
                  "Mirror the working channel's DAC, filter, volume, amplifier, and return wiring."));
  } else if (!status.left.dac_bus_complete ||
             !status.right.dac_bus_complete) {
    char bus_problem[256] = {};
    FormatString(bus_problem, sizeof(bus_problem),
                 "ui.wiring.help.incomplete_bus_fmt",
                 "The DAC route works, but its discrete buses are incomplete (L %d/16, R %d/16).",
                 CountDacBits(status.left.dac_connected_mask),
                 CountDacBits(status.right.dac_connected_mask));
    ImGui::TextColored(ImVec4(0.95f, 0.62f, 0.12f, 1.0f), "%s",
                       bus_problem);
    HelpBullet(TR("ui.wiring.help.need_all_bits",
                  "Connect every DAC output bit through the intended resistor network."));
  } else if (status.left.dac_weight_error_percent > 0.5f ||
             status.right.dac_weight_error_percent > 0.5f) {
    ImGui::TextColored(
        ImVec4(0.95f, 0.45f, 0.10f, 1.0f), "%s",
        TR("ui.wiring.help.dac_weight_fault",
           "All DAC bits are connected, but the resistor ratios produce incorrect bit weights."));
    HelpBullet(TR("ui.wiring.help.need_dac_ratios",
                  "Compare every DAC branch resistance and tolerance with the intended ladder value."));
  } else if (status.left.amplifier_power_insufficient ||
             status.right.amplifier_power_insufficient) {
    ImGui::TextColored(
        ImVec4(0.95f, 0.45f, 0.10f, 1.0f), "%s",
        TR("ui.wiring.help.amp_underpowered",
           "The amplifier cannot deliver even half of the speaker's rated power."));
    HelpBullet(TR("ui.wiring.help.need_amp_supply",
                  "Increase supply voltage/current capacity or lower the amplifier gain and speaker power target."));
  } else if (status.left.amplifier_bias_fault ||
             status.right.amplifier_bias_fault ||
             status.left.amplifier_thermal_tracking_fault ||
             status.right.amplifier_thermal_tracking_fault) {
    ImGui::TextColored(
        ImVec4(0.95f, 0.45f, 0.10f, 1.0f), "%s",
        TR("ui.wiring.help.amp_bias_fault",
           "The Class-AB output bias is incorrect or lacks thermal tracking."));
    HelpBullet(TR("ui.wiring.help.need_bias_fix",
                  "Use two suitable bias drops and thermally couple them to the output transistor heatsink."));
  } else if (status.left.amplifier_bandwidth_limited ||
             status.right.amplifier_bandwidth_limited) {
    ImGui::TextColored(
        ImVec4(0.95f, 0.45f, 0.10f, 1.0f), "%s",
        TR("ui.wiring.help.amp_bandwidth_fault",
           "A driver has less than 20 kHz closed-loop bandwidth."));
    HelpBullet(TR("ui.wiring.help.need_driver_bandwidth",
                  "Raise gain-bandwidth or reduce closed-loop gain, then check the response graph."));
  } else if (status.diagnosis.find("Stereo component mismatch") !=
             std::string::npos) {
    ImGui::TextColored(
        ImVec4(0.95f, 0.45f, 0.10f, 1.0f), "%s",
        TR("ui.wiring.help.stereo_mismatch",
           "Left and right frequency responses differ by more than 1 dB."));
    HelpBullet(TR("ui.wiring.help.need_filter_match",
                  "Match both channels' resistor, capacitor, inductor, and potentiometer values."));
  } else if (status.thd_percent > 5.0f) {
    ImGui::TextColored(
        ImVec4(0.95f, 0.45f, 0.10f, 1.0f), "%s",
        TR("ui.wiring.help.high_distortion",
           "The route works, but modeled distortion exceeds 5%."));
    HelpBullet(TR("ui.wiring.help.need_distortion_check",
                  "Inspect DAC accuracy, bias, slew rate, output loading, and the live FFT."));
  } else {
    ImGui::TextColored(
        ImVec4(0.15f, 0.70f, 0.28f, 1.0f), "%s",
        TR("ui.wiring.help.circuit_ok",
           "The powered stereo route and speaker protection are complete."));
  }

  if (!status.structured_diagnostics.empty()) {
    ImGui::Spacing();
    ImGui::Separator();
    ImGui::TextUnformatted("MNA v2 diagnostics");
    const size_t visible_count =
        std::min<size_t>(status.structured_diagnostics.size(), 6);
    for (size_t index = 0; index < visible_count; ++index) {
      const AudioCircuitDiagnostic& diagnostic =
          status.structured_diagnostics[index];
      ImVec4 color{0.35f, 0.55f, 0.75f, 1.0f};
      if (diagnostic.severity == AudioDiagnosticSeverity::WARNING) {
        color = {0.95f, 0.62f, 0.12f, 1.0f};
      } else if (diagnostic.severity ==
                     AudioDiagnosticSeverity::ERROR_LEVEL ||
                 diagnostic.severity == AudioDiagnosticSeverity::FATAL) {
        color = {0.90f, 0.25f, 0.18f, 1.0f};
      }
      ImGui::TextColored(color, "[%s]", diagnostic.code.c_str());
      ImGui::SameLine();
      ImGui::TextWrapped("%s", diagnostic.message.c_str());
      if (diagnostic.component_instance_id >= 0) {
        ImGui::TextDisabled("Component %d / pin %d",
                            diagnostic.component_instance_id,
                            diagnostic.port_id);
      }
    }
  }
  ...
  ImGui::Separator();
  ImGui::TextUnformatted(
      TR("ui.wiring.help.tutorial", "Quick build order"));
  HelpBullet(TR("ui.wiring.help.step1",
                "1. PC Digital Audio -> DAC I2S inputs."));
  HelpBullet(TR("ui.wiring.help.step2",
                "2. DAC output bits -> resistor ladder -> reconstruction LPF/buffer."));
  HelpBullet(TR("ui.wiring.help.step3",
                "3. Buffer -> 10k-100k volume potentiometer -> voltage driver."));
  HelpBullet(TR("ui.wiring.help.step4",
                "4. Powered driver -> bias network -> complementary NPN/PNP output stage."));
  HelpBullet(TR("ui.wiring.help.step5",
                "5. Emitter resistors -> large DC-blocking capacitor -> speaker."));
  HelpBullet(TR("ui.wiring.help.step6",
                "6. Connect all returns, build RTL, then start simulation and inspect Signal Analyzer."));
...
void Application::RenderWiringCanvas() {
  ...
  if (ImGui::BeginChild("Canvas", ImVec2(0, 0), true, canvas_flags)) {
    ...
    const float layout_scale = GetLayoutScale();
    const float help_button_size = 30.0f * layout_scale;
    const float help_button_padding = 12.0f * layout_scale;
    const ImVec2 help_button_min{
        canvas_top_left_.x + canvas_size_.x - help_button_size -
            help_button_padding,
        canvas_top_left_.y + help_button_padding};
    const ImVec2 help_button_max{help_button_min.x + help_button_size,
                                 help_button_min.y + help_button_size};
    RegisterOverlayInputCaptureRect(help_button_min, help_button_max, true);
    ...
    const AudioCircuitStatus circuit_status = GetAudioCircuitStatus();
    const bool circuit_has_problem =
        !placed_components_.empty() &&
        (!circuit_status.route_complete || !circuit_status.return_complete ||
         circuit_status.left.speaker_damage_risk ||
         circuit_status.right.speaker_damage_risk ||
         !circuit_status.left.route_complete ||
         !circuit_status.right.route_complete ||
         !circuit_status.left.dac_bus_complete ||
         !circuit_status.right.dac_bus_complete);
    ImGui::SetCursorScreenPos(help_button_min);
    ImGui::PushStyleVar(ImGuiStyleVar_FrameRounding, help_button_size * 0.5f);
    ImGui::PushStyleVar(ImGuiStyleVar_FrameBorderSize, 1.5f * layout_scale);
    ImGui::PushStyleColor(
        ImGuiCol_Button,
        circuit_has_problem ? ImVec4(0.95f, 0.48f, 0.10f, 0.96f)
                            : ImVec4(0.16f, 0.47f, 0.82f, 0.94f));
    ImGui::PushStyleColor(
        ImGuiCol_ButtonHovered,
        circuit_has_problem ? ImVec4(1.0f, 0.58f, 0.16f, 1.0f)
                            : ImVec4(0.22f, 0.58f, 0.96f, 1.0f));
    ImGui::PushStyleColor(ImGuiCol_ButtonActive,
                          ImVec4(0.12f, 0.36f, 0.68f, 1.0f));
    ImGui::PushStyleColor(ImGuiCol_Border,
                          ImVec4(1.0f, 1.0f, 1.0f, 0.90f));
    ImGui::Button("?##canvas_circuit_help", ImVec2(help_button_size,
                                                    help_button_size));
    const bool circuit_help_hovered = ImGui::IsItemHovered();
    ImGui::PopStyleColor(4);
    ImGui::PopStyleVar(2);
    if (circuit_help_hovered) {
      RenderCanvasCircuitHelpTooltip(placed_components_, circuit_status,
                                     layout_scale);
    }
    ...
    if (!handled) {
      ...
      if (top_index != -1) {
        ...
        if (comp.type == ComponentType::RTL_MODULE) {
          CanvasInteractionCommand command;
          command.type = CanvasInteractionCommand::Type::OpenRtlComponentEditor;
          ...
          if (behavior && behavior->OnDoubleClick) {
            ...
            ComponentInteractionResult interaction = behavior->OnDoubleClick(
          ...
          if (!handled_by_behavior) {
            CanvasInteractionCommand command;
            command.type =
                CanvasInteractionCommand::Type::OpenComponentContextMenu;
            command.component_id = comp.instanceId;
...
void Application::ApplyCanvasContextCommand(
  ...
  switch (command.type) {
    case CanvasInteractionCommand::Type::OpenComponentContextMenu:
      ...
      break;
    case CanvasInteractionCommand::Type::OpenRtlComponentEditor:
      OpenRtlComponentEditor(command.component_id);
      break;
    default:
      break;
  }
...
void Application::RenderComponentContextMenu() {
  ...
  if (ImGui::BeginPopup("Component Context")) {
    ...
    if (component_id != -1) {
      ...
      const bool is_passive_audio_component =
          comp && (comp->type == ComponentType::AUDIO_RESISTOR ||
                   comp->type == ComponentType::AUDIO_POTENTIOMETER ||
                   comp->type == ComponentType::AUDIO_CAPACITOR ||
                   comp->type == ComponentType::AUDIO_INDUCTOR);
      if (is_passive_audio_component &&
          ImGui::BeginMenu("Passive Component Properties")) {
        const char* default_name = "PASSIVE COMPONENT";
        switch (comp->type) {
          case ComponentType::AUDIO_RESISTOR:
            default_name = "RESISTOR";
            break;
          case ComponentType::AUDIO_POTENTIOMETER:
            default_name = "POTENTIOMETER";
            break;
          case ComponentType::AUDIO_CAPACITOR:
            default_name = "CAPACITOR";
            break;
          case ComponentType::AUDIO_INDUCTOR:
            default_name = "INDUCTOR";
            break;
          default:
            break;
        }
        ...
        std::snprintf(name_buffer, sizeof(name_buffer), "%s",
                      comp->customLabel.empty() ? default_name
                                                : comp->customLabel.c_str());
        const bool name_changed =
            ImGui::InputText("Display Name", name_buffer,
                             sizeof(name_buffer));
        if (ImGui::IsItemActivated()) {
          PushWiringUndoState();
        }
        if (name_changed) {
          comp->customLabel = name_buffer;
        }
        if (!comp->customLabel.empty() &&
            ImGui::Button("Reset Name")) {
          PushWiringUndoState();
          comp->customLabel.clear();
        }

        bool electrical_value_changed = false;
        if (comp->type == ComponentType::AUDIO_RESISTOR ||
            comp->type == ComponentType::AUDIO_POTENTIOMETER) {
          float resistance = std::max(
              0.001f,
              comp->internalStates.count(state_keys::kResistanceOhms)
                  ? comp->internalStates[state_keys::kResistanceOhms]
                  : (comp->type == ComponentType::AUDIO_POTENTIOMETER
                         ? 10000.0f
                         : 1000.0f));
          const bool submitted = ImGui::InputFloat(
              "Resistance (ohm)", &resistance, 1.0f, 100.0f, "%.3f",
              ImGuiInputTextFlags_EnterReturnsTrue);
          if (ImGui::IsItemActivated()) {
            PushWiringUndoState();
          }
          if (submitted || ImGui::IsItemDeactivatedAfterEdit()) {
            comp->internalStates[state_keys::kResistanceOhms] =
                std::max(0.001f, resistance);
            electrical_value_changed = true;
          }
        }
        if (comp->type == ComponentType::AUDIO_RESISTOR) {
          float tolerance = std::clamp(
              comp->internalStates.count(state_keys::kTolerancePercent)
                  ? comp->internalStates[state_keys::kTolerancePercent]
                  : 1.0f,
              0.0f, 20.0f);
          const bool submitted = ImGui::InputFloat(
              "Tolerance (%)", &tolerance, 0.001f, 0.1f, "%.4f",
              ImGuiInputTextFlags_EnterReturnsTrue);
          if (ImGui::IsItemActivated()) PushWiringUndoState();
          if (submitted || ImGui::IsItemDeactivatedAfterEdit()) {
            comp->internalStates[state_keys::kTolerancePercent] =
                std::clamp(tolerance, 0.0f, 20.0f);
            electrical_value_changed = true;
          }
        }
        if (comp->type == ComponentType::AUDIO_POTENTIOMETER) {
          float wiper_percent = 100.0f * std::clamp(
              comp->internalStates.count(state_keys::kWiperPosition)
                  ? comp->internalStates[state_keys::kWiperPosition]
                  : 0.5f,
              0.0f, 1.0f);
          const bool submitted = ImGui::InputFloat(
              "Wiper Position (%)", &wiper_percent, 1.0f, 10.0f, "%.2f",
              ImGuiInputTextFlags_EnterReturnsTrue);
          if (ImGui::IsItemActivated()) {
            PushWiringUndoState();
          }
          if (submitted || ImGui::IsItemDeactivatedAfterEdit()) {
            comp->internalStates[state_keys::kWiperPosition] =
                std::clamp(wiper_percent, 0.0f, 100.0f) / 100.0f;
            electrical_value_changed = true;
          }
        }
        if (comp->type == ComponentType::AUDIO_CAPACITOR) {
          float capacitance = std::max(
              0.000001f,
              comp->internalStates.count(state_keys::kCapacitanceUf)
                  ? comp->internalStates[state_keys::kCapacitanceUf]
                  : 10.0f);
          const bool submitted = ImGui::InputFloat(
              "Capacitance (uF)", &capacitance, 0.01f, 1.0f, "%.6f",
              ImGuiInputTextFlags_EnterReturnsTrue);
          if (ImGui::IsItemActivated()) {
            PushWiringUndoState();
          }
          if (submitted || ImGui::IsItemDeactivatedAfterEdit()) {
            comp->internalStates[state_keys::kCapacitanceUf] =
                std::max(0.000001f, capacitance);
            electrical_value_changed = true;
          }
        }
        if (comp->type == ComponentType::AUDIO_INDUCTOR) {
          float inductance = std::max(
              0.000001f,
              comp->internalStates.count(state_keys::kInductanceMh)
                  ? comp->internalStates[state_keys::kInductanceMh]
                  : 10.0f);
          const bool submitted = ImGui::InputFloat(
              "Inductance (mH)", &inductance, 0.01f, 1.0f, "%.6f",
              ImGuiInputTextFlags_EnterReturnsTrue);
          if (ImGui::IsItemActivated()) {
            PushWiringUndoState();
          }
          if (submitted || ImGui::IsItemDeactivatedAfterEdit()) {
            comp->internalStates[state_keys::kInductanceMh] =
                std::max(0.000001f, inductance);
            electrical_value_changed = true;
          }
        }
        if (electrical_value_changed) {
          OnElectricalConfigChanged();
        }
        ImGui::EndMenu();
      }
      if (comp && comp->type == ComponentType::AUDIO_SPEAKER &&
          ImGui::BeginMenu("Speaker Properties")) {
        bool speaker_changed = false;
        float impedance = comp->internalStates.count(state_keys::kImpedanceOhms)
                              ? comp->internalStates[state_keys::kImpedanceOhms]
                              : 8.0f;
        const bool impedance_submitted = ImGui::InputFloat(
            "Nominal Impedance (ohm)", &impedance, 0.1f, 1.0f, "%.2f",
            ImGuiInputTextFlags_EnterReturnsTrue);
        if (ImGui::IsItemActivated()) PushWiringUndoState();
        if (impedance_submitted || ImGui::IsItemDeactivatedAfterEdit()) {
          comp->internalStates[state_keys::kImpedanceOhms] =
              std::clamp(impedance, 0.1f, 1000.0f);
          speaker_changed = true;
        }
        float rated_power =
            comp->internalStates.count(state_keys::kMaximumPowerWatts)
                ? comp->internalStates[state_keys::kMaximumPowerWatts]
                : 100.0f;
        const bool power_submitted = ImGui::InputFloat(
            "Rated Power Per Channel (W)", &rated_power, 1.0f, 10.0f,
            "%.1f", ImGuiInputTextFlags_EnterReturnsTrue);
        if (ImGui::IsItemActivated()) PushWiringUndoState();
        if (power_submitted || ImGui::IsItemDeactivatedAfterEdit()) {
          comp->internalStates[state_keys::kMaximumPowerWatts] =
              std::clamp(rated_power, 0.1f, 100000.0f);
          speaker_changed = true;
        }
        ImGui::Separator();
        ImGui::TextUnformatted("Transducer response (parametric EQ)");
        ...
        constexpr std::array<const char*, 4> kGainKeys = {
            state_keys::kTransducerEq1GainDb,
            state_keys::kTransducerEq2GainDb,
            state_keys::kTransducerEq3GainDb,
            state_keys::kTransducerEq4GainDb};
        ...
        for (size_t band = 0; band < kFrequencyKeys.size(); ++band) {
          ImGui::PushID(static_cast<int>(band));
          ImGui::Text("Band %d", static_cast<int>(band + 1));
          auto edit_response_value = [&](const char* label, const char* key,
                                         float fallback, float minimum,
                                         float maximum, float step,
                                         const char* format) {
            float value = comp->internalStates.count(key)
                              ? comp->internalStates[key]
                              : fallback;
            const bool submitted = ImGui::InputFloat(
                label, &value, step, step * 10.0f, format,
                ImGuiInputTextFlags_EnterReturnsTrue);
            if (ImGui::IsItemActivated()) PushWiringUndoState();
            if (submitted || ImGui::IsItemDeactivatedAfterEdit()) {
              comp->internalStates[key] =
          ...
          edit_response_value("Frequency (Hz)", kFrequencyKeys[band],
                              kDefaultFrequencies[band], 10.0f, 22000.0f,
                              10.0f, "%.1f");
          edit_response_value("Gain (dB)", kGainKeys[band], 0.0f, -24.0f,
                              24.0f, 0.25f, "%+.2f");
          edit_response_value("Q", kQKeys[band], 1.0f, 0.1f, 20.0f,
                              0.05f, "%.2f");
          ImGui::PopID();
        }
        if (speaker_changed) OnElectricalConfigChanged();
        ImGui::EndMenu();
      }
      const bool is_audio_amplifier =
          comp &&
          (comp->type == ComponentType::AUDIO_OP_AMP ||
           (comp->type == ComponentType::RTL_MODULE &&
            comp->rtlModuleId == "audio_shell_amplifier"));
      if (is_audio_amplifier &&
          ImGui::BeginMenu("Voltage Amplifier Properties")) {
        bool amplifier_changed = false;
        auto edit_amplifier_value = [&](const char* label, const char* key,
                                        float fallback, float minimum,
                                        float maximum, float step,
                                        const char* format) {
          float value = comp->internalStates.count(key)
          ...
          if (ImGui::IsItemActivated()) PushWiringUndoState();
          if (submitted || ImGui::IsItemDeactivatedAfterEdit()) {
            comp->internalStates[key] = std::clamp(value, minimum, maximum);
            amplifier_changed = true;
          }
        };
        edit_amplifier_value("Closed-loop Gain", state_keys::kGain,
                             10.0f, 0.01f, 1000.0f, 0.1f, "%.3f");
        edit_amplifier_value("Open-loop Gain", state_keys::kOpenLoopGain,
                             100000.0f, 1.0f, 1000000000.0f, 1000.0f,
                             "%.0f");
        edit_amplifier_value("Gain-Bandwidth (Hz)",
                             state_keys::kGainBandwidthHz,
                             1000000.0f, 1.0f, 1000000000.0f, 1000.0f,
                             "%.0f");
        edit_amplifier_value("Slew Rate (V/us)",
                             state_keys::kSlewRateVoltsPerUs,
                             0.5f, 0.001f, 10000.0f, 0.1f, "%.3f");
        edit_amplifier_value("Input Offset (mV)",
                             state_keys::kInputOffsetMillivolts,
                             2.0f, -1000.0f, 1000.0f, 0.1f, "%.3f");
        edit_amplifier_value("Output Current Limit (A)",
                             state_keys::kOutputCurrentLimitAmps,
                             0.025f, 0.0f, 100.0f, 0.005f, "%.4f");
        edit_amplifier_value("Output Resistance (ohm)",
                             state_keys::kOutputResistanceOhms,
                             50.0f, 0.0f, 1000000.0f, 0.1f, "%.4f");
        edit_amplifier_value("Output Headroom (V)",
                             state_keys::kOutputHeadroomVolts,
                             1.0f, 0.0f, 1000.0f, 0.1f, "%.3f");
        ImGui::Separator();
        ImGui::TextWrapped(
            "This is a voltage amplifier/driver, not a speaker power stage. "
            "Wire V+/V-, then add complementary NPN/PNP emitter followers.");
        if (amplifier_changed) OnElectricalConfigChanged();
        ImGui::EndMenu();
      }
      const bool is_audio_semiconductor =
          comp && (comp->type == ComponentType::AUDIO_DIODE ||
                   comp->type == ComponentType::AUDIO_BJT_NPN ||
                   comp->type == ComponentType::AUDIO_BJT_PNP);
      if (is_audio_semiconductor &&
          ImGui::BeginMenu("Semiconductor Properties")) {
        bool semiconductor_changed = false;
        auto edit_semiconductor_value =
            [&](const char* label, const char* key, float fallback,
              ...
              if (submitted || ImGui::IsItemDeactivatedAfterEdit()) {
                comp->internalStates[key] =
                    std::clamp(value, minimum, maximum);
                semiconductor_changed = true;
              }
            };
        if (comp->type == ComponentType::AUDIO_DIODE) {
          edit_semiconductor_value("Forward Voltage (V)",
                                   state_keys::kForwardVoltageVolts,
                                   0.65f, 0.0f, 20.0f, 0.01f);
          edit_semiconductor_value("Dynamic Resistance (ohm)",
                                   state_keys::kDynamicResistanceOhms,
                                   2.0f, 0.0f, 1000000.0f, 0.1f);
          bool thermal_coupling =
              comp->internalStates.count(state_keys::kThermalCoupling) &&
              comp->internalStates[state_keys::kThermalCoupling] >= 0.5f;
          if (ImGui::Checkbox("Thermally Coupled to Output Heatsink",
                              &thermal_coupling)) {
            PushWiringUndoState();
            comp->internalStates[state_keys::kThermalCoupling] =
                thermal_coupling ? 1.0f : 0.0f;
            semiconductor_changed = true;
          }
          ImGui::TextWrapped(
              "Two silicon bias diodes normally reduce class-AB crossover "
              "distortion. Wrong bias is intentionally audible.");
        } else {
          edit_semiconductor_value("Current Gain (beta)",
                                   state_keys::kCurrentGainBeta,
                                   100.0f, 1.0f, 10000.0f, 1.0f, "%.1f");
          edit_semiconductor_value("Base-Emitter Voltage (V)",
                                   state_keys::kBaseEmitterVoltageVolts,
                                   0.65f, 0.0f, 10.0f, 0.01f);
          edit_semiconductor_value("Saturation Voltage (V)",
                                   state_keys::kSaturationVoltageVolts,
                                   0.2f, 0.0f, 10.0f, 0.01f);
          edit_semiconductor_value("Maximum Collector Current (A)",
                                   state_keys::kMaximumCollectorCurrentAmps,
                                   1.5f, 0.0f, 1000.0f, 0.05f);
          edit_semiconductor_value("Maximum Dissipation (W)",
                                   state_keys::kMaximumPowerWatts,
                                   1.0f, 0.0f, 10000.0f, 0.1f);
          edit_semiconductor_value("Thermal Resistance (C/W)",
                                   state_keys::kThermalResistanceCPerW,
                                   62.5f, 0.0f, 10000.0f, 1.0f, "%.1f");
          edit_semiconductor_value("Emitter Output Resistance (ohm)",
                                   state_keys::kOutputResistanceOhms,
                                   0.2f, 0.0f, 1000000.0f, 0.01f,
                                   "%.4f");
        }
        if (semiconductor_changed) OnElectricalConfigChanged();
        ImGui::EndMenu();
      }
      const bool is_source_component =
          comp && (comp->type == ComponentType::AUDIO_DC_SOURCE ||
                   comp->type == ComponentType::AUDIO_AC_SOURCE ||
                   comp->type == ComponentType::AUDIO_PULSE_SOURCE);
      if (is_source_component && ImGui::BeginMenu("Source Properties")) {
        const char* default_name =
            comp->type == ComponentType::AUDIO_DC_SOURCE
                ? "DC POWER"
                : (comp->type == ComponentType::AUDIO_AC_SOURCE
                       ? "AC / EQ SWEEP"
                       : "PULSE GENERATOR");
        ...
        const bool name_changed =
            ImGui::InputText("Display Name", name_buffer,
                             sizeof(name_buffer));
        if (ImGui::IsItemActivated()) PushWiringUndoState();
        if (name_changed) comp->customLabel = name_buffer;

        bool source_value_changed = false;
        auto edit_source_value = [&](const char* label, const char* key,
                                     float fallback, float minimum,
                                     float maximum, float step,
                                     const char* format = "%.3f") {
          float value = comp->internalStates.count(key)
          ...
          if (submitted || ImGui::IsItemDeactivatedAfterEdit()) {
            comp->internalStates[key] = std::clamp(value, minimum, maximum);
            source_value_changed = true;
          }
        };

        if (comp->type == ComponentType::AUDIO_DC_SOURCE) {
          edit_source_value("Voltage (V)", state_keys::kVoltageVolts,
                            5.0f, -1000.0f, 1000.0f, 0.1f);
          edit_source_value("Current Limit (A)",
                            state_keys::kCurrentLimitAmps,
                            1.0f, 0.0f, 1000.0f, 0.05f);
        } else if (comp->type == ComponentType::AUDIO_AC_SOURCE) {
          edit_source_value("RMS Voltage (V)", state_keys::kVoltageVolts,
                            1.0f, 0.0f, 1000.0f, 0.1f);
          edit_source_value("Tone Frequency (Hz)", state_keys::kFrequencyHz,
                            1000.0f, 0.01f, 1000000.0f, 10.0f, "%.2f");
          edit_source_value("Phase (deg)", state_keys::kPhaseDegrees,
                            0.0f, -360.0f, 360.0f, 1.0f, "%.2f");
          ImGui::Separator();
          ImGui::TextDisabled("EQ Sweep Range");
          edit_source_value("Sweep Start (Hz)",
                            state_keys::kFrequencyMinHz,
                            20.0f, 0.01f, 1000000.0f, 10.0f, "%.2f");
          edit_source_value("Sweep End (Hz)",
                            state_keys::kFrequencyMaxHz,
                            20000.0f, 0.01f, 1000000.0f, 100.0f, "%.2f");
        } else {
          edit_source_value("Low Voltage (V)", state_keys::kPulseLowVolts,
                            0.0f, -1000.0f, 1000.0f, 0.1f);
          edit_source_value("High Voltage (V)", state_keys::kPulseHighVolts,
                            5.0f, -1000.0f, 1000.0f, 0.1f);
          edit_source_value("Pulse Frequency (Hz)", state_keys::kFrequencyHz,
                            1000.0f, 0.01f, 1000000.0f, 10.0f, "%.2f");
          edit_source_value("Duty Cycle (%)",
                            state_keys::kDutyCyclePercent,
                            50.0f, 0.01f, 99.99f, 1.0f, "%.2f");
          ImGui::Separator();
          ImGui::TextDisabled("Frequency Sweep Range");
          edit_source_value("Sweep Start (Hz)",
                            state_keys::kFrequencyMinHz,
                            20.0f, 0.01f, 1000000.0f, 10.0f, "%.2f");
          edit_source_value("Sweep End (Hz)",
        ...
        if (source_value_changed) {
          if (comp->type != ComponentType::AUDIO_DC_SOURCE) {
            float& sweep_start =
                comp->internalStates[state_keys::kFrequencyMinHz];
            float& sweep_end =
                comp->internalStates[state_keys::kFrequencyMaxHz];
            if (sweep_start > sweep_end) {
              std::swap(sweep_start, sweep_end);
            }
          }
          if (comp->type == ComponentType::AUDIO_PULSE_SOURCE) {
            float& low = comp->internalStates[state_keys::kPulseLowVolts];
            float& high = comp->internalStates[state_keys::kPulseHighVolts];
