...
bool IsRtlComponentListable(const RtlLibraryEntry& entry) {
  // ADC and OP-AMP already have native audio component implementations.
  // Keep only the RTL DAC shell in the component palette; the native
  // AUDIO_OP_AMP remains available through the normal component registry.
  if (entry.moduleId == "audio_shell_adc" ||
      entry.moduleId == "audio_shell_amplifier") {
    return false;
  }
  return entry.componentEnabled;
}
...
const std::vector<ComponentType>& GetFunctionAxisOrder() {
  static const std::vector<ComponentType> kFunctionAxis = {
      ...
      ComponentType::AUDIO_RESISTOR,
      ComponentType::AUDIO_POTENTIOMETER,
      ComponentType::AUDIO_CAPACITOR,
      ComponentType::AUDIO_INDUCTOR,
      ComponentType::AUDIO_DIODE,
      ComponentType::AUDIO_BJT_NPN,
      ComponentType::AUDIO_BJT_PNP,
      ComponentType::AUDIO_OP_AMP,
      ComponentType::AUDIO_SPEAKER,
      ComponentType::AUDIO_GROUND,
      ComponentType::AUDIO_DC_SOURCE,
      ComponentType::AUDIO_AC_SOURCE,
      ComponentType::AUDIO_PULSE_SOURCE,
      ComponentType::PLC,
...
void Application::RenderPlacedComponents(ImDrawList* draw_list) {
  // [PPT: ??? ???] for??? ?????? ???????? ?????????????????.
  // Component movement does not alter draw order. Cache the sorted indices so
  // large circuit canvases do not pay O(N log N) every rendered frame.
  static const Application* cached_owner = nullptr;
  static std::vector<size_t> draw_order;
  static std::uint64_t cached_order_signature = 0;
  std::uint64_t order_signature = 0xcbf29ce484222325ULL;
  for (const PlacedComponent& component : placed_components_) {
    order_signature ^= static_cast<std::uint64_t>(component.instanceId) +
                       0x9e3779b97f4a7c15ULL +
                       (order_signature << 6U) + (order_signature >> 2U);
    order_signature ^= static_cast<std::uint64_t>(component.z_order) +
                       0x9e3779b97f4a7c15ULL +
                       (order_signature << 6U) + (order_signature >> 2U);
  }
  if (cached_owner != this || draw_order.size() != placed_components_.size() ||
      cached_order_signature != order_signature) {
    cached_owner = this;
    cached_order_signature = order_signature;
    draw_order.resize(placed_components_.size());
    for (size_t i = 0; i < placed_components_.size(); ++i) draw_order[i] = i;
      ...
      if (rtl_project_manager_ &&
        ...
        for (const auto& rtl_entry : rtl_project_manager_->GetLibrary()) {
          ...
          if (ImGui::BeginChild("RtlComponentIcon", ImVec2(card_size, card_size),
            ...
            previewComp.rtlModuleId = rtl_entry.moduleId;
            const ComponentDefinition* shellAppearance =
                GetAudioRtlShellAppearance(rtl_entry.moduleId);
            if (rtl_entry.analyzeSuccess && rtl_entry.buildSuccess) {
              previewComp.runtimePorts =
                  RtlProjectManager::BuildRuntimePorts(rtl_entry.ports);
            }
            previewComp.size = shellAppearance
                                   ? shellAppearance->size
                                   : (rtlDef ? rtlDef->size
                                             : Size{260.0f, 180.0f});
            if (rtl_entry.analyzeSuccess && rtl_entry.buildSuccess &&
                !shellAppearance) {
              ...
              previewComp.size.height =
                  std::max(140.0f,
                           92.0f + 18.0f * static_cast<float>(maxSideCount));
            }
            if (shellAppearance && !previewComp.runtimePorts.empty()) {
              const size_t inputCount = std::count_if(
                  previewComp.runtimePorts.begin(),
                  previewComp.runtimePorts.end(),
                  [](const Port& port) { return port.isInput; });
              const size_t outputCount =
                  previewComp.runtimePorts.size() - inputCount;
              const size_t maxSideCount =
                  std::max(inputCount, outputCount);
              if (maxSideCount > 1) {
                if (maxSideCount > 8) {
                  previewComp.size.width =
                      std::max(previewComp.size.width, 190.0f);
                }
                previewComp.size.height = std::max(
                    previewComp.size.height,
                    56.0f + 18.0f * static_cast<float>(maxSideCount - 1));
              }
              size_t inputIndex = 0;
              size_t outputIndex = 0;
              const float topMargin =
                  std::min(28.0f, previewComp.size.height * 0.28f);
              const float usableHeight = std::max(
                  1.0f, previewComp.size.height - topMargin * 2.0f);
              for (Port& port : previewComp.runtimePorts) {
                const size_t count = port.isInput ? inputCount : outputCount;
                ...
                const float fraction =
                    count > 1
                        ...
                        : 0.5f;
                port.relativePos.x =
                    port.isInput ? 0.0f : previewComp.size.width;
                port.relativePos.y = topMargin + usableHeight * fraction;
                ++index;
              }
            }
