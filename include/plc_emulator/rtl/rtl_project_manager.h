...
class RtlProjectManager {
 public:
  ...
  // Dedicated ADC/DAC/OP-AMP Verilog belongs to the open project. Restore
  // their zero-pin editor shells before loading another project.
  void ResetProjectAudioComponentShells();
  ...
  // Thread-safe detached operations. They only mutate the supplied snapshot
  // and build-cache files; the live UI library is committed on the main
  // thread by Application::PollRtlAsyncTask().
  bool AnalyzeDetached(RtlLibraryEntry* entry) const;
  bool BuildDetached(RtlLibraryEntry* entry) const;
  bool RunTestbenchDetached(RtlLibraryEntry* entry) const;
 ...
 private:
  ...
  void EnsureAudioComponentShells();
